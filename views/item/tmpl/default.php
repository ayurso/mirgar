...
<?php	
		...
		echo '<div class="title_top info"><p style="font-weight:500; font-size:1.5rem;">Опубликовано '.date('d-m-Y',$date_start).' в '.date('H:i',$date_start).', обращение <span style="color:tomato;"> № '.$item->id.'</span></p>';
	
    	echo '<h2>'.$item->name.'</h2>'; 
    	echo '<div class="category"><span class="label_title">Тема: &nbsp;</span>';
    	echo '<a href="'.DJClassifiedsSEO::getCategoryRoute($item->cat_id.':'.$item->c_alias).'" >'.$item->c_name.'</a>'.'</div>';
	...
	<div class="dj-item-in">
		...
		<div class="item-answer">
	        ...
	        <div class="description"><h2>Официальный ответ службы/организации</h2></div> 
			<div class="djcf_images_generaldet_box">	
				<?php
		        if($item->answer){
			        ...
			        echo '<div class="intro_answer col-md-8"><p style="font-weight:600; ">Опубликовано: &nbsp;'.date('d-m-Y',strtotime($item->date_answer)).'</p>';
			        echo '<p style="font-weight:600; ">Исполнитель: &nbsp;</span>'.$this->org_name.'</p>';
			        echo '<p>'.$item->answer.'</p></div>';
		        }
		        else{
		        echo '<div class="intro_answer">Готовится официальный ответ</div>';
		        }?>
