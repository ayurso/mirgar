...
class DJclassifiedsViewItem extends JViewLegacy{
	...
	function display($tpl = null){		
		...
		$item_images2 = null;
		if($item->answer)
		    $item_images2 = DJClassifiedsImage::getAdsImages($item->id,"1");
		 
		
		$category=$model->getCategory($item->cat_id); 
		
		$org_name=$category->idOrg;
		
		if($category->idOrg > 0){
		  
		  $org=$model->getOrganisation($category->idOrg);
		  $org_name= $org->nameOrg;
		} 
		 else{ 
		   $cat_parent=$model->getCategory($category->parent_id);
		   if($cat_parent->idOrg > 0){
		       $org=$model->getOrganisation($cat_parent->idOrg);
		       $org_name= $org->nameOrg;
		   }
		   else
		     $org_name='Не определён';
		...
		if($item->user_id!=0){
		    ...
		    if($item_images2){
				$document->addCustomTag('<meta property="og:image" content="'.rtrim($u->toString(),'/').$item_images2[0]->thumb_b.'" />');
			
				$path = JPath::clean(JPATH_ROOT . '/' .$item_images2[0]->thumb_b);
				$size = @getimagesize($path);
				if(isset($size[0])){
					$document->addCustomTag('<meta property="og:image:width" content="'.$size[0].'" />');
				}
		...
		$this->assignRef('item_images2',$item_images2);
		$this->assignRef('item_options',$item_options);
		$this->assignRef('org_name', $org_name);
