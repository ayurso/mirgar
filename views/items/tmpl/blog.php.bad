...
if($main_id>0 || $main_rid>0 || $se>0 || JRequest::getInt('fav','0') || $uid>0 || ($main_id==0 && $par->get('items_in_main_cat',1)) ){
	...
	<?php 		
		...
		foreach($this->items as $i){
			...
			echo '<div class="title"><div class="row"><div class="col-md-9">';

				echo '<h2><a href="'.DJClassifiedsSEO::getItemRoute($i->id.':'.$i->alias,$i->cat_id.':'.$i->c_alias,$i->region_id.':'.$i->r_name,$this->main_cat,$i->extra_cats).$anch.'" >'.$i->name.'</a></h2></div><div class="col-md-3">';
				if($i->user_id && isset($i->profile['details']->verified)){
					if($i->profile['details']->verified==1){
						echo '<span class="verified_icon col-xs-1" title="'.JText::_('COM_DJCLASSIFIEDS_VERIFIED_SELLER').'" ></span>';
					}
				}
			...
			<?php } 			
			echo '<div class="row no-gutters"><div class="col-sm-12 col-md-3">';	
				...
				if($par->get('blog_desc_position','right')=='right'){
					...
					if($par->get('blog_desc_position','right')=='bottom'){	
						...
						echo '<div class="item_box_bottom">';		
							echo '<div class="item_desc"><span class="label_title">'.JText::_('COM_DJCLASSIFIEDS_DESCRIPTION').': &nbsp;</span>';
								echo '<span>'.mb_substr(strip_tags($i->intro_desc), 0,$par->get('introdesc_char_limit','120'),'UTF-8');
							echo '</span></div>';					
						echo '</div>';
						if($i->event->afterDJClassifiedsDisplayContent) { ?>
							<div class="djcf_after_content">
