...
if($main_id>0 || $main_rid>0 || $se>0 || JRequest::getInt('fav','0') || $uid>0 || ($main_id==0 && $par->get('items_in_main_cat',1)) ){
	...
	<?php 		
		...
		foreach($this->items as $i){
			...
			echo '<div class="row title "><div class="col-xs-12 col-md-9">';
				echo '<h2><a href="'.DJClassifiedsSEO::getItemRoute($i->id.':'.$i->alias,$i->cat_id.':'.$i->c_alias,$i->region_id.':'.$i->r_name,$this->main_cat,$i->extra_cats).$anch.'" >'.$i->name.'</a></h2></div><div class="col-xs-12 col-md-3">';
				if($i->user_id && isset($i->profile['details']->verified)){
					if($i->profile['details']->verified==1){
			...
			<?php } 			
			echo '</div><div class="row"><div class="col-xs-3 col-md-3">';	
				...
				if($par->get('blog_desc_position','right')=='right'){
					...
					if(($par->get('blog_image','1')==1 && count($i->images)) || $par->get('blog_image','1')==2){
						...
						echo '</a>';
						echo '</div></div><div class="col-xs-12 col-md-9">';
					}																						
				...
				if($par->get('blog_desc_position','right')=='right'){
					...
					echo '<div class="item_desc"><span class="label_title">'.JText::_('COM_DJCLASSIFIEDS_DESCRIPTION').'</span>';
						echo '<span class="desc_info">'.mb_substr(strip_tags($i->intro_desc), 0,$par->get('introdesc_char_limit','150'),'UTF-8');
					echo '</span></div>';
					...
					if($par->get('column_date_a','1')){
						echo '<div class="date_start"><b><span class="label_title">Обращение </span><span style="color:Tomato">№ '.$i->id.' </span> от </b> '.date('d.m.Y',strtotime($i->date_start)).'</div>';
					}
					if($par->get('blog_category','0')){
						echo '<div class="category"><span class="label_title">Тема: &nbsp;</span>';
					...
					if($par->get('blog_location','1') && $i->r_name && $par->get('show_regions','1')){
						echo '<div class="region"><span class="label_title">Место: &nbsp;</span><a href="'.DJClassifiedsSEO::getRegionRoute($i->region_id.':'.$i->r_name).'">'.$i->r_name.'</a></div>';
					}
					...
					/*if($par->get('column_date_a','1')){
						echo '<div class="date_start"><span class="label_title">Дата </span>'.$i->date_start.'</div>';
					}*/							
					...
					//	echo '<div class="item_box_bottom">';		
							echo '<div class="item_desc"><span class="label_title">'.JText::_('COM_DJCLASSIFIEDS_DESCRIPTION').': &nbsp;</span>';
								echo '<span>'.mb_substr(strip_tags($i->intro_desc), 0,$par->get('introdesc_char_limit','200'),'UTF-8');
							echo '</span></div>';					
						echo '</div>';
						if($i->event->afterDJClassifiedsDisplayContent) { ?>
							<div class="djcf_after_content">
					...
					if($par->get('blog_profile','1') && $i->user_id){
