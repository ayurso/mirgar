...
<div id="dj-classifieds" class="clearfix djcftheme-<?php echo $this->theme;?><?php echo $pageclass_sfx;?>">
	<?php 
			...
			if($main_id){
				...
				if($modules_djcf!==null && count($modules_djcf)>0){
					echo '<div class="djcf-ad-top-cat clearfix">';
					foreach (array_keys($modules_djcf) as $m){
						echo JModuleHelper::renderModule($modules_djcf[$m],$mod_attribs);
					}
	...
	if($se>0){
		...
		if($par->get('title_in_items','1')){				
			if($main_id>0){
                ...
                if ( $main_id<=25 )
                {
        		    echo '<h1 class="main_cat_title">'.'<img src="'.JURI::base(true).$this->cat_imag[$this->main_cat->id]->path.$this->cat_imag[$this->main_cat->id]->name.'_ths.'.$this->cat_imag[$this->main_cat->id]->ext.'" style="display: inline;" /> '.$this->main_cat->name;
                }
                else
                {
